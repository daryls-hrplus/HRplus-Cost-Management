...
CL003,Regional Services Ltd,V5,Current Billing + Percentage,Workforce|Payroll|Leave,300,USD,5000,7.5,2026-01-01,,Active,Increase current USD billing by 7.5 percent`;
  ...
  function renderTables() {
    ...
    $("clientTableBody").innerHTML = state.filteredClients.map((client) => { const pricingResult = evaluateClientPricing(client), gap = pricingResult.pricingGapPerEmployeeTTD, interpretation = gap === null ? "Incomplete" : gap > 0 ? `${money2.format(gap)} more per employee` : gap < 0 ? `${money2.format(Math.abs(gap))} less per employee` : "On target"; return `<tr><td><strong>${esc(client.clientName)}</strong><div class="item-sub">${esc(client.clientId)}</div></td><td>${esc(client.project)} · ${esc(client.status)}</td><td>${interpretation}${pricingResult.pricingGapPercent === null ? "" : `<div class="item-sub">Gap: ${pricingResult.pricingGapPercent.toFixed(2)}%</div>`}</td><td>${client.employeeCount}</td><td>${client.currency} ${money2.format(client.monthlyBillingAmount)}<br>TTD ${money2.format(pricingResult.actualMonthlyRevenueTTD)}</td><td>TTD ${pricingResult.actualRatePerEmployeeTTD === null ? "—" : money2.format(pricingResult.actualRatePerEmployeeTTD)}</td><td>${pricingResult.expectedRatePerEmployeeTTD === null ? "Incomplete" : "TTD " + money2.format(pricingResult.expectedRatePerEmployeeTTD)}<div class="item-sub">${esc(pricingResult.pricingInterpretation)}</div></td><td>${esc(pricingResult.pricingStatus)}</td><td>${esc(client.pricingModel)}</td><td>${client.modules.map((module) => `<span class="pill">${esc(module)}</span>`).join(" ")}</td><td><button class="btn" type="button" data-client-detail="${client.id}">View</button></td></tr>`; }).join("");
  }
